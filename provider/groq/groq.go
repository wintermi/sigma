...
// Provider adapts Groq's OpenAI-compatible Chat Completions endpoint to sigma.
type Provider = openai.Provider

// ProviderOption configures a Provider.
type ProviderOption = openai.ProviderOption

// NewProvider constructs a Groq provider.
...
// Register adds a Groq text provider to registry.
func Register(registry *sigma.Registry, opts ...ProviderOption) error {
	...
	return registry.RegisterTextProvider(sigma.ProviderGroq, NewProvider(opts...))
}

// RegisterDefault adds a Groq text provider to sigma's default registry.
func RegisterDefault(opts ...ProviderOption) error {
	return sigma.RegisterDefaultTextProvider(sigma.ProviderGroq, NewProvider(opts...))
}
