...
// Provider adapts DeepSeek's OpenAI-compatible Chat Completions endpoint to sigma.
type Provider = openai.Provider

// ProviderOption configures a Provider.
type ProviderOption = openai.ProviderOption

// NewProvider constructs a DeepSeek provider.
...
// Register adds a DeepSeek text provider to registry.
func Register(registry *sigma.Registry, opts ...ProviderOption) error {
	...
	return registry.RegisterTextProvider(sigma.ProviderDeepSeek, NewProvider(opts...))
}

// RegisterDefault adds a DeepSeek text provider to sigma's default registry.
func RegisterDefault(opts ...ProviderOption) error {
	return sigma.RegisterDefaultTextProvider(sigma.ProviderDeepSeek, NewProvider(opts...))
}
