...
// Provider adapts Cerebras's OpenAI-compatible Chat Completions endpoint to sigma.
type Provider = openai.Provider

// ProviderOption configures a Provider.
type ProviderOption = openai.ProviderOption

// NewProvider constructs a Cerebras provider.
...
// Register adds a Cerebras text provider to registry.
func Register(registry *sigma.Registry, opts ...ProviderOption) error {
	...
	return registry.RegisterTextProvider(sigma.ProviderCerebras, NewProvider(opts...))
}

// RegisterDefault adds a Cerebras text provider to sigma's default registry.
func RegisterDefault(opts ...ProviderOption) error {
	return sigma.RegisterDefaultTextProvider(sigma.ProviderCerebras, NewProvider(opts...))
}
