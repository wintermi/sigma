...
const (
	defaultVertexOpenAIAPIVersion = "v1beta1"
...
func (p *VertexProvider) run(ctx context.Context, writer sigma.StreamWriter, model sigma.Model, req sigma.Request, opts sigma.Options) {
	...
	compat := openAICompletionsCompat(model, p.baseURLForModel(model, opts))
	grammarTools, err := chatGrammarToolInputProperties(req, opts, compat)
	if err != nil {
		final.StopReason = sigma.StopReasonError
		_ = writer.Error(ctx, err, final)
		return
	}
	final, err = parseCompletionsStream(ctx, body, writer, model, grammarTools, compat.supportsFinishReason)
	if err != nil {
		if errors.Is(err, context.Canceled) || errors.Is(err, context.DeadlineExceeded) || ctx.Err() != nil {
			final.StopReason = sigma.StopReasonAborted
			_ = writer.Error(ctx, contextError(ctx, err), final)
...
func (p *VertexProvider) newRequest(ctx context.Context, model sigma.Model, req sigma.Request, opts sigma.Options) (*http.Request, error) {
	...
	httpReq.Header.Set("User-Agent", "sigma/vertex-openai-completions")
...
func (p *VertexProvider) addProviderHeaders(req *http.Request, provider sigma.ProviderID, opts sigma.Options, compat completionsCompat) {
...
func (p *VertexProvider) requestConfig(model sigma.Model, opts sigma.Options) (vertexRequestConfig, error) {
	...
	if config.APIVersion == "" {
		config.APIVersion = defaultVertexOpenAIAPIVersion
	}
...
func (p *VertexProvider) endpoint(model sigma.Model, opts sigma.Options, config vertexRequestConfig) (string, error) {
	...
	return baseURL + "/" + vertexai.ProjectLocation(config.vertexConfig()) + "/endpoints/openapi/chat/completions", nil
}

func invalidVertexOptions(model sigma.Model, message string, err error) error {
	return fmt.Errorf("vertex openai config: %w", vertexai.InvalidOptions(model, message, err))
}
