...
// Provider adapts OpenRouter's OpenAI-compatible Chat Completions endpoint to sigma.
type Provider = openai.Provider

// ProviderOption configures a Provider.
type ProviderOption = openai.ProviderOption

// NewProvider constructs an OpenRouter text provider.
...
// Register adds an OpenRouter text provider to registry.
func Register(registry *sigma.Registry, opts ...ProviderOption) error {
	...
	return registry.RegisterTextProvider(sigma.ProviderOpenRouter, NewProvider(opts...))
}

// RegisterDefault adds an OpenRouter text provider to sigma's default registry.
func RegisterDefault(opts ...ProviderOption) error {
	return sigma.RegisterDefaultTextProvider(sigma.ProviderOpenRouter, NewProvider(opts...))
}
