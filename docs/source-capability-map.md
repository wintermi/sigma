...
| Qwen Token Plan OpenAI-compatible Chat Completions | `openai-completions` | [provider/qwen](../provider/qwen), [provider/openai](../provider/openai), `Model.OpenAICompletionsCompat` | Regional convenience wrappers over the shared Chat Completions adapter; focused generated metadata includes Qwen3.7 Max and Qwen3.8 Max with Qwen reasoning compatibility. |
...
| GitHub Copilot | `github-copilot` | `openai-completions`, `openai-responses`, `anthropic-messages` | Use [provider/githubcopilot](../provider/githubcopilot). Generated metadata includes Copilot OpenAI-compatible and Anthropic-compatible routes; caller-owned OAuth credentials can explicitly enable policies and discover account-available models for advisory catalog filtering. |
