...
| Radius gateway Messages | `radius-messages` | [provider/radius](../provider/radius), `TextModelSource`, `RadiusOAuthTokenProvider` | Runtime text adapter with native SSE streaming, image/thinking/tool replay, caller-configured browser/device OAuth, and API-key or OAuth-authenticated catalog refresh. |
...
| Radius gateway | `radius` | `radius-messages` | Use [provider/radius](../provider/radius) and explicitly refresh gateway-owned models. `RADIUS_API_KEY` remains the API-key fallback; caller-configured OAuth can use provider auth and `WithCatalogAuthResolver`. No static or durable catalog cache is provided. |
