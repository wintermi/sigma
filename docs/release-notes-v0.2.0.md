...
`sigma` v0.2.0 adds a runtime adapter for OpenAI Images generation. The existing
generated `gpt-image-1` metadata is now runnable when applications register the
...
No root package JSON shapes changed. The public API addition is limited to new
registration helpers in `provider/openai`.
...
The OpenAI Images adapter is covered by deterministic `httptest` fixtures and a
golden request payload; no live OpenAI network calls are required.
