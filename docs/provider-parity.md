...
| GitHub Copilot compatible text routes | `openai-completions`, `openai-responses`, `anthropic-messages` | `preview` | `fixture-tested` | `partial` | `not supported by provider` | `fixture-tested` | `partial` | `partial` | `partial` | `partial` | `partial` | `implemented` | `fixture-tested` | `fixture-tested` | `fixture-tested` | `intentionally omitted` |
...
| Radius gateway Messages | `radius-messages` | `preview` | `fixture-tested` | `fixture-tested` | `not supported by provider` | `fixture-tested` | `fixture-tested` | `fixture-tested` | `fixture-tested` | `fixture-tested` | `fixture-tested` | `fixture-tested` | `fixture-tested` | `fixture-tested` | `fixture-tested` | `intentionally omitted` |
...
- GitHub Copilot compatible text routes: [provider/githubcopilot/githubcopilot_test.go](../provider/githubcopilot/githubcopilot_test.go), [provider/githubcopilot/oauth_test.go](../provider/githubcopilot/oauth_test.go), [provider/openai/completions_test.go](../provider/openai/completions_test.go), [provider/openai/responses_test.go](../provider/openai/responses_test.go).
...
- OpenAI-compatible Chat Completions supports typed `tool_choice`, opt-in
  ...
  arbitrary sampling parameters. Custom compatible models can also opt into
  safely clamped top-level thinking-token budgets; GitHub Copilot and
...
- Qwen Token Plan uses the shared OpenAI-compatible Chat Completions adapter
  ...
  curated seven-model catalog with model-specific input and thinking metadata.
  Broader catalog coverage and provider-specific hosted-tool behavior remain
  deferred.
...
- GitHub Copilot includes device-code OAuth login, token refresh, explicit
