...
var (
	...
	jsonSecretPattern        = regexp.MustCompile(`(?i)("(?:(?:api[_-]?key)|(?:access[_-]?token)|(?:refresh[_-]?token)|(?:id[_-]?token)|(?:client[_-]?secret)|(?:device[_-]?code)|(?:user[_-]?code)|(?:secret[_-]?access[_-]?key)|(?:session[_-]?token)|(?:provider[_-]?signature)|authorization|cookie|signature)"[ \t\r\n]*:[ \t\r\n]*)("[^"\\]*(?:\\[\s\S][^"\\]*)*(?:"|\\?\z)|null|true|false|-?[0-9]+(?:\.[0-9]+)?)`)
