...
- [x] Clamp typed OpenAI Responses-compatible and Azure OpenAI Responses
      output-token limits to the accepted request minimum while preserving
      low-level override precedence and Codex omission.
...
- [x] Add the China-only GLM-4.6V vision model and apply API-equivalent GLM-5.1
      and GLM-5V-Turbo usage estimates across both Z.ai Coding Plan routes.
- [x] Harden OpenAI-compatible Chat Completions streams to avoid duplicate
      ...
      `finish_reason`.
- [x] Preserve explicitly supplied typed OpenAI-compatible Chat Completions tool
      choices when no tool definitions are emitted, including fully deferred
      tool sets, while leaving omitted choices absent and preserving
      declared-tool behavior and low-level payload precedence.
...
- [x] Add model-scoped default arbitrary sampling parameters for
      OpenAI-compatible Chat Completions, Responses, and Azure Responses with
      typed request, request-scoped sampling, and raw `extra_body` precedence.
...
OpenAI-compatible Chat Completions adapter, plus Grok 4.5 and Grok 4.6 over
the shared OpenAI Responses adapter. Curated generated metadata covers the
direct xAI routes. Future additions should still be promoted only with
...
- [x] Add direct Grok 4.6 Responses metadata with text/image input, tiered
      long-context pricing, `xhigh` reasoning, and deterministic request
      coverage.
